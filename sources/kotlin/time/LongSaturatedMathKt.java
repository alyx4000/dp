...
public final class LongSaturatedMathKt {
    ...
    public static final long m2697saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long m2614toLongimpl = Duration.m2614toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m2696checkInfiniteSumDefinedPjuGub4(j, j2, m2614toLongimpl);
        }
        if ((1 | (m2614toLongimpl - 1)) == Long.MAX_VALUE) {
            return m2698saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + m2614toLongimpl;
        return ((j ^ j3) & (m2614toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m2696checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m2600isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m2698saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long m2571divUwyO8pc = Duration.m2571divUwyO8pc(j2, 2);
        long m2614toLongimpl = Duration.m2614toLongimpl(m2571divUwyO8pc, durationUnit);
        return ((1 | (m2614toLongimpl - 1)) > Long.MAX_VALUE ? 1 : ((1 | (m2614toLongimpl - 1)) == Long.MAX_VALUE ? 0 : -1)) == 0 ? m2614toLongimpl : m2697saturatingAddNuflL3o(m2697saturatingAddNuflL3o(j, durationUnit, m2571divUwyO8pc), durationUnit, Duration.m2603minusLRDsOJo(j2, m2571divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m2670getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m2669getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m2621unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m2604plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m2621unaryMinusUwyO8pc(infinityOfSign(j3));
        }
