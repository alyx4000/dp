...
public final class DescriptorBasedTypeSignatureMappingKt {
    ...
    public static final <T> T mapType(KotlinType kotlinType, JvmTypeFactory<T> factory, TypeMappingMode mode, TypeMappingConfiguration<? extends T> typeMappingConfiguration, JvmDescriptorTypeWriter<T> jvmDescriptorTypeWriter, Function3<? super KotlinType, ? super T, ? super TypeMappingMode, Unit> writeGenericType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            ...
            if (alternativeType == null) {
                alternativeType = typeMappingConfiguration.commonSupertype(intersectionTypeConstructor.mo2508getSupertypes());
            }
        ...
        ClassifierDescriptor classifierDescriptor = mo2507getDeclarationDescriptor;
        if (ErrorUtils.isError(classifierDescriptor)) {
            T t2 = (T) factory.createObjectType("error/NonExistentClass");
            typeMappingConfiguration.processErrorType(kotlinType, (ClassDescriptor) mo2507getDeclarationDescriptor);
            if (jvmDescriptorTypeWriter != 0) {
                jvmDescriptorTypeWriter.writeClass(t2);
            }
            return t2;
        }
        boolean z = mo2507getDeclarationDescriptor instanceof ClassDescriptor;
        ...
        if (z) {
            ...
            if (mode.isForAnnotationParameter() && KotlinBuiltIns.isKClass((ClassDescriptor) mo2507getDeclarationDescriptor)) {
                t = (Object) factory.getJavaLangClassType();
            } else {
                ClassDescriptor classDescriptor = (ClassDescriptor) mo2507getDeclarationDescriptor;
        ...
        if (mo2507getDeclarationDescriptor instanceof TypeParameterDescriptor) {
            KotlinType representativeUpperBound = TypeUtilsKt.getRepresentativeUpperBound((TypeParameterDescriptor) mo2507getDeclarationDescriptor);
            ...
            if (jvmDescriptorTypeWriter != 0) {
                Name name = mo2507getDeclarationDescriptor.getName();
                Intrinsics.checkNotNullExpressionValue(name, "descriptor.getName()");
                jvmDescriptorTypeWriter.writeTypeVariable(name, t3);
            }
            return t3;
        }
        if ((mo2507getDeclarationDescriptor instanceof TypeAliasDescriptor) && mode.getMapTypeAliases()) {
            return (T) mapType(((TypeAliasDescriptor) mo2507getDeclarationDescriptor).getExpandedType(), factory, mode, typeMappingConfiguration, jvmDescriptorTypeWriter, writeGenericType);
        }
    ...
    public static final String computeInternalName(ClassDescriptor klass, TypeMappingConfiguration<?> typeMappingConfiguration) {
        ...
        if (containingDeclaration instanceof PackageFragmentDescriptor) {
            ...
            return sb.append(StringsKt.replace$default(asString, '.', IOUtils.DIR_SEPARATOR_UNIX, false, 4, (Object) null)).append(IOUtils.DIR_SEPARATOR_UNIX).append(identifier).toString();
        }
