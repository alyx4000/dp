...
public final class BufferFrameLoader implements FrameLoader {
    ...
    public FrameResult getFrame(int frameNumber, int width, int height) {
        ...
        if (bufferFrame != null) {
            ...
            return new FrameResult(bufferFrame.getBitmapRef().mo382clone(), FrameResult.FrameType.SUCCESS);
        }
        loadNextFrames(width, height);
    ...
    private final FrameResult findNearestToRender(int targetFrame) {
        ...
        if (findNearestFrame != null) {
            CloseableReference<Bitmap> mo382clone = findNearestFrame.getBitmap().mo382clone();
            Intrinsics.checkNotNullExpressionValue(mo382clone, "nearestFrame.bitmap.clone()");
            this.lastRenderedFrameNumber = findNearestFrame.getFrameNumber();
            return new FrameResult(mo382clone, FrameResult.FrameType.NEAREST);
        }
