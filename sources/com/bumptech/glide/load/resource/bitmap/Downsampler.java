        ...
        if (shouldUsePool(imageType)) {
            if (i6 < 0 || i7 < 0 || !z2) {
                ...
                if (Log.isLoggable(TAG, 2)) {
                    Log.v(TAG, "Calculated target [" + round + ViewHierarchyNode.JsonKeys.X + round2 + "] for source [" + i6 + ViewHierarchyNode.JsonKeys.X + i7 + "], sampleSize: " + i9 + ", targetDensity: " + options.inTargetDensity + ", density: " + options.inDensity + ", density multiplier: " + f);
                }
            } else {
                round = i4;
                round2 = i5;
