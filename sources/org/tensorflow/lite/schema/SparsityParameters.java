...
public final class SparsityParameters extends Table {
    ...
    public void unpackTo(SparsityParametersT sparsityParametersT) {
        ...
        for (int i2 = 0; i2 < blockMapLength(); i2++) {
            iArr2[i2] = blockMap(i2);
        }
        sparsityParametersT.setBlockMap(iArr2);
        DimensionMetadataT[] dimensionMetadataTArr = new DimensionMetadataT[dimMetadataLength()];
        for (int i3 = 0; i3 < dimMetadataLength(); i3++) {
            dimensionMetadataTArr[i3] = dimMetadata(i3) != null ? dimMetadata(i3).unpack() : null;
        }
        sparsityParametersT.setDimMetadata(dimensionMetadataTArr);
    }

    public static int pack(FlatBufferBuilder flatBufferBuilder, SparsityParametersT sparsityParametersT) {
        int i = 0;
        if (sparsityParametersT == null) {
            return 0;
        }
        int createTraversalOrderVector = sparsityParametersT.getTraversalOrder() != null ? createTraversalOrderVector(flatBufferBuilder, sparsityParametersT.getTraversalOrder()) : 0;
        int createBlockMapVector = sparsityParametersT.getBlockMap() != null ? createBlockMapVector(flatBufferBuilder, sparsityParametersT.getBlockMap()) : 0;
        if (sparsityParametersT.getDimMetadata() != null) {
            ...
            int i2 = 0;
            while (i < length) {
                iArr[i2] = DimensionMetadata.pack(flatBufferBuilder, dimMetadata[i]);
                i2++;
                i++;
            }
            i = createDimMetadataVector(flatBufferBuilder, iArr);
        }
        return createSparsityParameters(flatBufferBuilder, createTraversalOrderVector, createBlockMapVector, i);
    }
}
