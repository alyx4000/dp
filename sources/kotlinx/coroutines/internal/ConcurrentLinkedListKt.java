    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m2803constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m2803constructorimpl(CLOSED);
            }
        ...
        do {
            ...
            if (!SegmentOrClosed.m2808isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                    z = true;
                    if (segment2.id >= m2806getSegmentimpl.id) {
                        break;
                    }
                    ...
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment2, m2806getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
        ...
        do {
            ...
            if (!SegmentOrClosed.m2808isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (ChannelSegment$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceArray, i, segment2, m2806getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
                    } else if (m2806getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
