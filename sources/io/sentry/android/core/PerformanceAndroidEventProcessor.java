...
final class PerformanceAndroidEventProcessor implements EventProcessor {
    ...
    public synchronized SentryTransaction process(SentryTransaction sentryTransaction, Hint hint) {
        ...
        if (!this.sentStartMeasurement && hasAppStartSpan(sentryTransaction)) {
            ...
            if (durationMs != 0) {
                sentryTransaction.getMeasurements().put(AppStartMetrics.getInstance().getAppStartType() == AppStartMetrics.AppStartType.COLD ? MeasurementValue.KEY_APP_START_COLD : MeasurementValue.KEY_APP_START_WARM, new MeasurementValue(Float.valueOf(durationMs), MeasurementUnit.Duration.MILLISECOND.apiName()));
