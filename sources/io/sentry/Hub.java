...
public final class Hub implements IHub, MetricsApi.IMetricsInterface {
            ...
            if (z) {
                executorService.submit(new Runnable() { // from class: io.sentry.Hub$$ExternalSyntheticLambda3
                    ...
                    public final void run() {
                        Hub.this.m1147lambda$close$1$iosentryHub(executorService);
                    }
                });
            } else {
    ...
    /* synthetic */ void m1147lambda$close$1$iosentryHub(ISentryExecutorService iSentryExecutorService) {
    ...
    public void pushScope() {
        ...
        Stack.StackItem peek = this.stack.peek();
        this.stack.push(new Stack.StackItem(this.options, peek.getClient(), peek.getScope().m1152clone()));
    }
    ...
    private IScope buildLocalScope(IScope iScope, ScopeCallback scopeCallback) {
        if (scopeCallback != null) {
            try {
                IScope m1152clone = iScope.m1152clone();
                scopeCallback.run(m1152clone);
                return m1152clone;
            } catch (Throwable th) {
    ...
    public Map<String, String> getDefaultTagsForMetrics() {
        ...
        hashMap.put("release", this.options.getRelease());
        hashMap.put("environment", this.options.getEnvironment());
        return hashMap;
    }
}
