...
public final class BrotliInterceptor implements Interceptor {
    ...
    public okhttp3.Response intercept(Interceptor.Chain chain) {
        ...
        if (chain.request().header(HttpRequest.HEADER_ACCEPT_ENCODING) == null) {
            return uncompress$okhttp_brotli(chain.proceed(chain.request().newBuilder().header(HttpRequest.HEADER_ACCEPT_ENCODING, "br,gzip").build()));
        }
    ...
    public final okhttp3.Response uncompress$okhttp_brotli(okhttp3.Response response) {
        ...
        if (!HttpHeaders.promisesBody(response) || (body = response.body()) == null || (header$default = okhttp3.Response.header$default(response, HttpRequest.HEADER_CONTENT_ENCODING, null, 2, null)) == null) {
            return response;
        }
        if (StringsKt.equals(header$default, "br", true)) {
            ...
            if (!StringsKt.equals(header$default, HttpRequest.ENCODING_GZIP, true)) {
                return response;
            }
            buffer = Okio.buffer(new GzipSource(body.getBodySource()));
        }
        return response.newBuilder().removeHeader(HttpRequest.HEADER_CONTENT_ENCODING).removeHeader(HttpRequest.HEADER_CONTENT_LENGTH).body(ResponseBody.INSTANCE.create(buffer, body.get$contentType(), -1L)).build();
    }
}
